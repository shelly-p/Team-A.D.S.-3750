...
const Dashboard = () => {
  ...
  const handleAddPlayer = async () => {
    ...
    const existingPlayerQuery = query(
      ...
      where("gameID", "==", gameInfo.current.access),
  ...
  const handleCreateGame = async () => {
    ...
    const accessCode = await addGame(
      category,
      numOfQuestions,
      "user@example.com"
    );
    ...
    // Save the selected category and number of questions in session storage
    sessionStorage.setItem("category", category);
    sessionStorage.setItem("numOfQuestions", numOfQuestions);

    // Save the questions in session storage
